...
import { fetchAppMqttCertification, type EcoflowDeviceIdentity } from './ecoflow/appAuth.js';
...
function buildCatalogPayload(devices: EcoflowDeviceIdentity[]) {
  return {
    devices: devices
      .map((device) => ({
        deviceId: device.sn,
...
async function resolveCatalog(config: ReturnType<typeof loadConfig>): Promise<EcoflowDeviceIdentity[]> {
  let devices: EcoflowDeviceIdentity[] = [];
...
async function main(): Promise<void> {
  ...
  if (missingTargets.length > 0) {
    console.warn(
      `[bridge] migration targets missing from resolved catalog: ${missingTargets.join(', ')}. `
  ...
  const ingest = new MqttIngestService(config, store, {
    onDeviceDelta: (_deviceId, delta) => {
      ws.broadcastDeviceDelta(delta as never);
      ws.broadcastFleetState();
      ws.broadcastDeviceSnapshot((delta as { deviceId: string }).deviceId);
    },
