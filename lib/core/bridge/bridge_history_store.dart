...
class BridgeHistoryStore {
  ...
  Future<void> recordSnapshot(BridgeDeviceSnapshot snapshot) async {
    ...
    final points = _readPoints(snapshot.deviceId)
        .where((point) => !point.timestamp.isBefore(cutoff))
        .toList()
      ..sort((a, b) => a.timestamp.compareTo(b.timestamp));
    final bucketSizeMs = _sampleWindow.inMilliseconds;
    final nextBucket = nextPoint.timestamp.millisecondsSinceEpoch ~/ bucketSizeMs;
  ...
  DeviceHistoryPoint _pointFromSnapshot(BridgeDeviceSnapshot snapshot) {
    ...
    return DeviceHistoryPoint(
      ...
      batteryTempC: metricAsDouble('battery.maxCellTempC') ?? snapshot.temperatureC,
    );
  }
